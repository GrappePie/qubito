    ...
    <div className="bg-white p-2 rounded-lg flex items-center shadow-sm">
      <div className="flex-1">
        ...
        <div className="flex items-center mt-1 select-none">
          <button className="text-slate-500 hover:text-slate-800 px-2" onClick={() => dispatch(updateQuantity({ id, delta: -1 }))}>-</button>
          <input
            type="number"
            value={quantity}
            className="w-10 p-0!  text-center mx-1 border rounded text-sm"
            onChange={e => dispatch(updateQuantity({ id, quantity: Number(e.target.value) }))}
            min={1}
          />
          <button className="text-slate-500 hover:text-slate-800 px-2" onClick={() => dispatch(updateQuantity({ id, delta: 1 }))}>+</button>
        </div>
